    ...
    async getLogList(data) {
        ...
        return { 
            data:await this.app.mysql.query(
                `SELECT 
                g.id,g.user_id,g.user_name,g.operation_ip,g.operation_method,g.operation_desc,g.operation_url,g.operation_time 
                FROM fs_operation_logs g 
                WHERE g.operation_state=? AND g.id LIKE '%`+uid+`%' 
